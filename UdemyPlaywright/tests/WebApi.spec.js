//Login with UI
// 5 sub test cases -> apart from APi
//Storage Container => save all the details in json format
//Assignment --> Login into client side and fetch 1 element
const {test, expect}=require('@playwright/test')
//storing the locators
    let emailName = "gihiw10644@gxuzi.com";
let webContext;
...
test.beforeAll(async({browser})=>{
    ...
    let signButton = page.locator("#login");
     //enter the details and clicked
    await email.fill("gihiw10644@gxuzi.com");
    await password.fill("3hUh8rkg!8.Yxs$");
    await signButton.click();
    await page.locator(".card-body b").first().waitFor(); // only wa
    //capture all the cookies
    await context.storageState({path:'state.json'});
    webContext = await browser.newContext({storageState:'state.json'});
})

//to avoid manual context
//test.use({storageState: 'state.json'});
test('Login into client dashboard',async() =>{
    const page = await webContext.newPage();
    await page.goto("https://rahulshettyacademy.com/client");
    let productName = page.locator(".card-body");
